      ...
      <div class="space-y-4">
        <div>
          <label class="block text-[13px] text-gray-800 font-medium mb-1.5"
            >{{ $t('invoice_panel.ip_invoiceStatus') }} <span class="text-red-500">*</span></label
          >
          <select
          ...
          >
            <option value="unpaid">{{ $t('invoice.i_waitingStatus') }}</option>
          </select>
        </div>

        <div>
          <label class="block text-[13px] text-gray-800 font-medium mb-1.5"
            >{{ $t('invoice.i_period') }} <span class="text-red-500">*</span></label
          >
          <select
            v-model="editFormData.payment_type"
            class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-[13px] text-gray-700 focus:outline-none focus:border-[#2BB5F4] bg-white"
          >
        ...
        <div class="flex flex-col md:flex-row gap-4 pt-2">
          <div class="flex-1">
            <label class="block text-[13px] text-gray-800 font-medium mb-1.5">{{
              $t('invoice_panel.ip_startDate')
            }}</label>
            <input
              v-model="editFormData.active_from_date"
              type="date"
          ...
          <div class="flex-1">
            <label class="block text-[13px] text-gray-800 font-medium mb-1.5">{{
              $t('invoice_panel.ip_endDate')
            }}</label>
            <input
              v-model="editFormData.active_to_date"
              type="date"
              class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-[13px] text-gray-700 focus:outline-none focus:border-[#2BB5F4]"
            />
          </div>
